...
CREATE TRIGGER session_log_trigger AFTER INSERT OR DELETE ON auth.sessions FOR EACH ROW EXECUTE FUNCTION log_session_activity();

CREATE TRIGGER on_auth_user_created AFTER INSERT ON auth.users FOR EACH ROW EXECUTE FUNCTION handle_new_user();

CREATE TRIGGER on_auth_user_updated AFTER UPDATE ON auth.users FOR EACH ROW EXECUTE FUNCTION update_user_profile_on_auth_change();
