...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: profile, error: profileError } = await supabase.from('profiles').select('*').eq('id', user_id).single();
    if (profileError || !profile) {
      throw new Error('User profile not found');
    }
    let { data: stats } = await supabase.from('user_statistics').select('accepted_offers, avg_response_time_hours').eq('user_id', user_id).maybeSingle();
    if (!stats) {
    ...
    const totalScoreRaw = Object.entries(breakdown).reduce((total, [key, score])=>{
      const weight = TRUST_SCORE_WEIGHTS[key] ?? 0;
...
const calculateListingsScore = async (supabase, userId)=>{
  const { count } = await supabase.from('listings').select('*', {
    count: 'exact',
    head: true
