...
import { Send, Loader2, Package, DollarSign, User, Calendar, CheckCircle, XCircle, Clock, Trash2 } from 'lucide-react'
...
const SentOffersPage = () => {
  ...
  const handleDeleteOffer = async (offerId: string) => {
    ...
    try {
      const success = await deleteOffer(offerId, user.id)
      if (success) {
    ...
    return (
      <div className="min-h-[calc(100vh-80px)] flex flex-col items-center justify-center bg-background p-4 text-center">
        <EmptyStateList
          ...
          description="Gönderdiğiniz teklifleri görmek için giriş yapmanız gerekiyor."
          action={
  ...
  return (
    ...
    >
      <div className="mb-8">
        <h1 className="text-3xl md:text-4xl font-bold mb-2">Gönderdiğim Teklifler</h1>
        <p className="text-muted-foreground">
          İlanlara gönderdiğiniz teklifleri buradan görüntüleyebilir ve yönetebilirsiniz.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-4">
          {filteredOffers.map((offer) => (
            <OfferCard
              key={offer.id}
              offer={offer}
              onUpdateStatus={() => {}} // Not used for sent offers
              isUpdating={false}
              mode="sent"
              onDelete={offer.status === 'pending' ? handleDeleteOffer : undefined}
      ...
      {showEmptyState && (
        <div className="text-center py-20 bg-card rounded-2xl border">
          ...
          <p className="text-muted-foreground mb-8 max-w-md mx-auto">
            {filterStatus === 'all'
              ? 'Henüz hiçbir ilana teklif göndermediniz. İlanları inceleyip teklif göndermeye başlayabilirsiniz.'
