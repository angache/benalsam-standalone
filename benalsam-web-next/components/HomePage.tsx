  ...
  useEffect(() => {
    ...
    loadInitialListings();
      ...
      const handleSearchSubmit = useCallback((e) => {
        ...
        if (localSearchQuery.trim()) {
          ...
          router.push(`/arama?q=${encodeURIComponent(localSearchQuery)}`);
        }
      ...
      const handleAISuggestionClick = useCallback((suggestion) => {
        ...
        if (suggestion.type === 'category' && suggestion.category) {
          ...
          router.push(`/arama?q=${encodeURIComponent(suggestion.text)}`);
        }
    ...
    <div className="flex items-center gap-2 self-end lg:self-center">
      {import.meta.env.MODE !== 'production' && (() => {
        // Compute source counters from currently shown list
        const currentList = isAnyFilterActive ? displayedListings : initialListings;
        const unique = new Map();
        (currentList || []).forEach(l => { if (l?.id) unique.set(l.id, l); });
        let e = 0, s = 0;
        unique.forEach(v => { if (v && v.__src === 'E') e++; else if (v && v.__src === 'S') s++; });
        return (
