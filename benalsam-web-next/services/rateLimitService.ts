// Rate Limiting Service for Login Security
...
class RateLimitService {
  ...
  private getRateLimitData(email: string): RateLimitData {
    try {
  ...
  private saveRateLimitData(email: string, data: RateLimitData): void {
    try {
  ...
  async checkRateLimit(email: string): Promise<RateLimitResult> {
    ...
    if (data.blocked && data.blockExpiry && now < data.blockExpiry) {
      ...
      return {
        ...
        timeRemaining,
        message: `Hesap güvenlik nedeniyle kilitlendi. ${Math.ceil(timeRemaining / 60)} dakika sonra tekrar deneyin.`
      };
    }
    ...
    if (data.blocked && data.blockExpiry && now >= data.blockExpiry) {
      ...
      data.attempts = 0;
      this.saveRateLimitData(email, data);
    }
    ...
    // Check 1-hour window for account lockout
