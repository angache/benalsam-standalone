...
async function createListings() {
  const BATCH_SIZE = 10; // ✅ 10'ar 10'ar ekle
  
  // 🔍 Supabase'de kaç TEST ilanı var? (otomatik offset)
  ...
  const remainingListings = LISTINGS.length - offset
  const batchSize = Math.min(BATCH_SIZE, remainingListings)
  
  console.log(`🚀 Creating ${batchSize} realistic test listings...\n`)
  console.log(`📊 Total available: ${LISTINGS.length} listings`)
  console.log(`📊 Already created: ${offset} listings`)
  console.log(`📊 Remaining: ${remainingListings} listings`)
  console.log(`📊 This batch: ${offset + 1}-${offset + batchSize}\n`)
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n')

  if (batchSize === 0) {
    console.log('✅ All listings already created!')
    return
  }
  ...
  for (let i = 0; i < batchSize; i++) {
    ...
    try {
      process.stdout.write(`[${offset + i + 1}/${offset + batchSize}] ${listing.title.substring(0, 40)}... `)
