import { 
  Baby, 
  Bitcoin, 
  Book, 
  Briefcase, 
  Building, 
  Camera, 
  Car, 
  Dumbbell, 
  Gamepad2, 
  GraduationCap, 
  Home, 
  Laptop, 
  Music, 
  Palette, 
  Plane, 
  Shirt, 
  Smartphone, 
  Utensils, 
  Wrench,
  // Yeni ikonlar
  Monitor,
  Headphones,
  Tv,
  Gamepad,
  Tablet,
  Printer,
  Watch,
  Heart,
  Stethoscope,
  Scissors,
  Truck,
  Bus,
  Bike,
  Flower,
  Trees,
  ShoppingBag,
  ShoppingCart,
  DollarSign,
  Coins,
  TrendingUp,
  Users,
  User,
  UserCheck,
  MessageCircle,
  Calendar,
  Star,
  Gift,
  FileText,
  Brush,
  PenTool,
  Award
} from "lucide-react-native";

interface SubSubCategory {
  name: string;
}

interface SubCategory {
  name: string;
  icon?: any;
  subcategories?: SubSubCategory[];
}

interface Category {
  name: string;
  icon: any;
  color: string;
  subcategories: SubCategory[];
}

const categoriesConfigEnhanced = [
  {
    name: "Elektronik",
    icon: Smartphone,
    color: "from-blue-500 to-cyan-500",
    subcategories: [
      {
        name: "Telefon & Tablet",
        icon: Smartphone,
        subcategories: [
          { name: "iPhone" },
          { name: "Samsung" },
          { name: "Xiaomi" },
          { name: "Huawei" },
          { name: "Oppo" },
          { name: "Vivo" },
          { name: "OnePlus" },
          { name: "Diğer Akıllı Telefon" },
          { name: "Tuşlu Telefon" },
          { name: "iPad" },
          { name: "Samsung Tablet" },
          { name: "Diğer Tablet" },
          { name: "Telefon Aksesuarları" },
          { name: "Tablet Aksesuarları" },
        ],
      },
      {
        name: "Bilgisayar",
        icon: Laptop,
        subcategories: [
          { name: "MacBook" },
          { name: "Dell Laptop" },
          { name: "HP Laptop" },
          { name: "Lenovo Laptop" },
          { name: "Asus Laptop" },
          { name: "Acer Laptop" },
          { name: "MSI Laptop" },
          { name: "Diğer Dizüstü" },
          { name: "Mac Masaüstü" },
          { name: "Dell Masaüstü" },
          { name: "HP Masaüstü" },
          { name: "Lenovo Masaüstü" },
          { name: "Diğer Masaüstü" },
          { name: "İşlemci (CPU)" },
          { name: "Ekran Kartı (GPU)" },
          { name: "Anakart" },
          { name: "RAM" },
          { name: "SSD & HDD" },
          { name: "Güç Kaynağı" },
          { name: "Kasa" },
          { name: "Monitör" },
          { name: "Klavye" },
          { name: "Mouse" },
          { name: "Hoparlör" },
          { name: "Kulaklık" },
          { name: "Webcam" },
          { name: "Yazıcı" },
          { name: "Tarayıcı" },
          { name: "Fotokopi Makinesi" },
        ],
      },
      {
        name: "TV & Ses Sistemleri",
        icon: Tv,
        subcategories: [
          { name: "Televizyon" },
          { name: "Ses Sistemleri" },
          { name: "Kulaklık & Hoparlör" },
          { name: "Projeksiyon" },
        ],
      },
      {
        name: "Oyun & Eğlence",
        icon: Gamepad,
        subcategories: [
          { name: "Oyun Konsolu" },
          { name: "Oyun Aksesuarları" },
          { name: "Video Oyunları" },
          { name: "Masa Oyunları" },
        ],
      },
      {
        name: "Fotoğraf & Kamera",
        icon: Camera,
        subcategories: [
          { name: "Dijital Kamera" },
          { name: "Video Kamera" },
          { name: "Kamera Aksesuarları" },
          { name: "Drone" },
        ],
      },
      {
        name: "Küçük Elektronik",
        icon: Watch,
        subcategories: [
          { name: "Saat & Takı" },
          { name: "Hesap Makinesi" },
          { name: "Elektronik Oyunlar" },
          { name: "Diğer" },
        ],
      },
    ],
  },
  {
    name: "Ev Aletleri & Mobilya",
    icon: Home,
    color: "from-green-500 to-emerald-500",
    subcategories: [
      {
        name: "Ev Aletleri",
        icon: Wrench,
        subcategories: [
          { name: "Çamaşır Makinesi & Kurutma" },
          { name: "Bulaşık Makinesi" },
          { name: "Buzdolabı & Dondurucu" },
          { name: "Fırın & Ocak" },
          { name: "Mikrodalga & Küçük Ev Aletleri" },
          { name: "Süpürge & Temizlik" },
          { name: "Isıtma & Soğutma" },
        ],
      },
      {
        name: "Mobilya",
        icon: ShoppingBag,
        subcategories: [
          { name: "Oturma Odası Mobilyası" },
          { name: "Yatak Odası Mobilyası" },
          { name: "Mutfak Mobilyası" },
          { name: "Çalışma Odası Mobilyası" },
          { name: "Çocuk Odası Mobilyası" },
          { name: "Bahçe Mobilyası" },
        ],
      },
      {
        name: "Dekorasyon",
        icon: Palette,
        subcategories: [
          { name: "Aydınlatma" },
          { name: "Perde & Stor" },
          { name: "Halı & Kilim" },
          { name: "Tablo & Resim" },
          { name: "Vazo & Süs Eşyaları" },
          { name: "Yastık & Örtü" },
        ],
      },
      {
        name: "Mutfak Eşyaları",
        icon: Utensils,
        subcategories: [
          { name: "Tencere & Tava" },
          { name: "Bardak & Tabak" },
          { name: "Çatal Bıçak Takımları" },
          { name: "Mutfak Aletleri" },
          { name: "Saklama Kapları" },
        ],
      },
      {
        name: "Bahçe & Yapı Market",
        icon: Flower,
        subcategories: [
          { name: "Bahçe Aletleri" },
          { name: "Bitki & Çiçek" },
          { name: "Yapı Malzemeleri" },
          { name: "El Aletleri" },
        ],
      },
    ],
  },
  {
    name: "Araç & Vasıta",
    icon: Car,
    color: "from-red-500 to-pink-500",
    subcategories: [
      {
        name: "Otomobil",
        icon: Car,
        subcategories: [
          { name: "Binek Araç" },
          { name: "SUV & Jip" },
          { name: "Ticari Araç" },
          { name: "Klasik Araç" },
          { name: "Antika Araç" },
          { name: "Modifiye Araç" },
          { name: "Lüks Araç" },
          { name: "Spor Araç" },
          { name: "Elektrikli Araç" },
          { name: "Hibrit Araç" },
        ],
      },
      {
        name: "Motosiklet",
        icon: Bike,
        subcategories: [
          { name: "Sport Motosiklet" },
          { name: "Cruiser" },
          { name: "Scooter" },
          { name: "Enduro" },
        ],
      },
      {
        name: "Bisiklet",
        icon: Bike,
        subcategories: [
          { name: "Şehir Bisikleti" },
          { name: "Dağ Bisikleti" },
          { name: "Yarış Bisikleti" },
          { name: "Elektrikli Bisiklet" },
        ],
      },
      {
        name: "Ticari Araçlar",
        icon: Truck,
        subcategories: [
          { name: "Kamyon" },
          { name: "Kamyonet" },
          { name: "Otobüs" },
          { name: "Çekici" },
        ],
      },
      {
        name: "Yedek Parça & Aksesuar",
        icon: Wrench,
        subcategories: [
          { name: "Motor Parçaları" },
          { name: "Kaporta Parçaları" },
          { name: "Elektronik Parçalar" },
          { name: "Aksesuarlar" },
        ],
      },
    ],
  },
  {
    name: "Emlak",
    icon: Building,
    color: "from-orange-400 to-amber-600",
    subcategories: [
      {
        name: "Konut",
        subcategories: [
          { name: "Satılık Daire" },
          { name: "Kiralık Daire" },
          { name: "Satılık Villa" },
          { name: "Kiralık Villa" },
          { name: "Satılık Müstakil Ev" },
          { name: "Kiralık Müstakil Ev" },
          { name: "Satılık Yazlık" },
          { name: "Kiralık Yazlık" },
          { name: "Satılık Prefabrik Ev" },
          { name: "Kiralık Prefabrik Ev" },
        ],
      },
      {
        name: "Ticari",
        subcategories: [
          { name: "Satılık Dükkan & Mağaza" },
          { name: "Kiralık Dükkan & Mağaza" },
          { name: "Satılık Ofis" },
          { name: "Kiralık Ofis" },
          { name: "Satılık Depo & Antrepo" },
          { name: "Kiralık Depo & Antrepo" },
          { name: "Satılık Fabrika" },
          { name: "Kiralık Fabrika" },
          { name: "Devren Satılık İşyeri" },
          { name: "Devren Kiralık İşyeri" },
        ],
      },
      {
        name: "Arsa & Tarla",
        subcategories: [
          { name: "İmarlı Arsa" },
          { name: "İmarsız Arsa" },
          { name: "Tarla" },
          { name: "Bahçe" },
          { name: "Bağ & Bahçe" },
          { name: "Zeytinlik" },
          { name: "Meyve Bahçesi" },
        ],
      },
      {
        name: "Turistik Tesis",
        subcategories: [
          { name: "Satılık Otel" },
          { name: "Kiralık Otel" },
          { name: "Satılık Pansiyon" },
          { name: "Kiralık Pansiyon" },
          { name: "Satılık Restoran" },
          { name: "Kiralık Restoran" },
          { name: "Satılık Cafe" },
          { name: "Kiralık Cafe" },
        ],
      },
      {
        name: "Projeler",
        subcategories: [
          { name: "Konut Projeleri" },
          { name: "Ticari Projeler" },
          { name: "Karma Projeler" },
          { name: "Turistik Projeler" },
        ],
      },
    ],
  },
  {
    name: "Moda & Giyim",
    icon: Shirt,
    color: "from-pink-500 to-rose-500",
    subcategories: [
      {
        name: "Kadın Giyim",
        icon: User,
        subcategories: [
          { name: "Elbise" },
          { name: "Gömlek" },
          { name: "Bluz" },
          { name: "Tişört" },
          { name: "Kazak" },
          { name: "Hırka" },
          { name: "Ceket" },
          { name: "Mont" },
          { name: "Kaban" },
          { name: "Pantolon" },
          { name: "Etek" },
          { name: "Şort" },
          { name: "İç Giyim" },
          { name: "Mayo & Bikini" },
          { name: "Spor Giyim" },
          { name: "Abiye" },
          { name: "Gelinlik" },
        ],
      },
      {
        name: "Erkek Giyim",
        icon: UserCheck,
        subcategories: [
          { name: "Gömlek" },
          { name: "Tişört" },
          { name: "Kazak" },
          { name: "Hırka" },
          { name: "Ceket" },
          { name: "Mont" },
          { name: "Kaban" },
          { name: "Pantolon" },
          { name: "Şort" },
          { name: "İç Giyim" },
          { name: "Spor Giyim" },
          { name: "Takım Elbise" },
          { name: "Smokin" },
          { name: "Forma" },
        ],
      },
      {
        name: "Çocuk Giyim",
        icon: Baby,
        subcategories: [
          { name: "Kız Çocuk" },
          { name: "Erkek Çocuk" },
          { name: "Bebek" },
        ],
      },
      {
        name: "Ayakkabı",
        icon: ShoppingCart,
        subcategories: [
          { name: "Kadın Ayakkabı" },
          { name: "Erkek Ayakkabı" },
          { name: "Çocuk Ayakkabı" },
          { name: "Spor Ayakkabı" },
        ],
      },
      {
        name: "Aksesuar",
        icon: Gift,
        subcategories: [
          { name: "Çanta" },
          { name: "Takı" },
          { name: "Saat" },
          { name: "Gözlük" },
          { name: "Şal & Eşarp" },
        ],
      },
    ],
  },
  {
    name: "Spor & Outdoor",
    icon: Dumbbell,
    color: "from-teal-500 to-cyan-500",
    subcategories: [
      {
        name: "Fitness & Egzersiz",
        icon: Heart,
        subcategories: [
          { name: "Kardiyo Ekipmanları" },
          { name: "Ağırlık Ekipmanları" },
          { name: "Yoga Ekipmanları" },
          { name: "Fitness Aksesuarları" },
        ],
      },
      {
        name: "Takım Sporları",
        icon: Users,
        subcategories: [
          { name: "Futbol" },
          { name: "Basketbol" },
          { name: "Voleybol" },
          { name: "Hentbol" },
          { name: "Tenis" },
          { name: "Badminton" },
          { name: "Masa Tenisi" },
          { name: "Squash" },
          { name: "Golf" },
          { name: "Beyzbol" },
          { name: "Rugby" },
          { name: "Hokey" },
        ],
      },
      {
        name: "Outdoor & Kamp",
        icon: Trees,
        subcategories: [
          { name: "Kamp Malzemeleri" },
          { name: "Dağcılık" },
          { name: "Balıkçılık" },
          { name: "Avcılık" },
          { name: "Su Sporları" },
        ],
      },
      {
        name: "Spor Giyim",
        icon: Shirt,
        subcategories: [
          { name: "Erkek Spor Giyim" },
          { name: "Kadın Spor Giyim" },
          { name: "Çocuk Spor Giyim" },
        ],
      },
    ],
  },
  {
    name: "Eğitim & Kitap",
    icon: GraduationCap,
    color: "from-purple-500 to-violet-500",
    subcategories: [
      {
        name: "Kitaplar",
        icon: Book,
        subcategories: [
          { name: "Roman & Hikaye" },
          { name: "Bilim & Teknoloji" },
          { name: "Tarih & Biyografi" },
          { name: "Çocuk Kitapları" },
          { name: "Ders Kitapları" },
        ],
      },
      {
        name: "Kurslar & Dersler",
        icon: Users,
        subcategories: [
          { name: "Matematik" },
          { name: "Fizik" },
          { name: "Kimya" },
          { name: "Biyoloji" },
          { name: "Türkçe" },
          { name: "İngilizce" },
          { name: "Almanca" },
          { name: "Fransızca" },
          { name: "İspanyolca" },
          { name: "Arapça" },
          { name: "Rusça" },
          { name: "Çince" },
          { name: "Japonca" },
          { name: "Korece" },
          { name: "YKS Hazırlık" },
          { name: "LGS Hazırlık" },
          { name: "ALES Hazırlık" },
          { name: "KPSS Hazırlık" },
          { name: "TOEFL Hazırlık" },
          { name: "IELTS Hazırlık" },
          { name: "Piyano Dersi" },
          { name: "Gitar Dersi" },
          { name: "Keman Dersi" },
          { name: "Resim Dersi" },
          { name: "Dans Dersi" },
          { name: "Yoga Dersi" },
          { name: "Pilates Dersi" },
          { name: "Yüzme Dersi" },
          { name: "Tenis Dersi" },
          { name: "Golf Dersi" },
          { name: "Sürüş Dersi" },
        ],
      },
      {
        name: "Kırtasiye",
        icon: PenTool,
        subcategories: [
          { name: "Kalem & Defter" },
          { name: "Çanta & Dosya" },
          { name: "Ofis Malzemeleri" },
          { name: "Sanat Malzemeleri" },
        ],
      },
    ],
  },
  {
    name: "Hizmetler",
    icon: Briefcase,
    color: "from-orange-500 to-amber-500",
    subcategories: [
      {
        name: "Tamir & Bakım",
        icon: Wrench,
        subcategories: [
          { name: "Ev Tadilat" },
          { name: "Teknik Servis" },
          { name: "Temizlik" },
          { name: "Nakliyat" },
        ],
      },
      {
        name: "Özel Ders",
        icon: GraduationCap,
        subcategories: [
          { name: "Matematik" },
          { name: "İngilizce" },
          { name: "Müzik" },
          { name: "Spor" },
          { name: "Diğer" },
        ],
      },
      {
        name: "Organizasyon",
        icon: Calendar,
        subcategories: [
          { name: "Düğün Organizasyonu" },
          { name: "Etkinlik Organizasyonu" },
          { name: "Kurumsal Organizasyon" },
        ],
      },
      {
        name: "Danışmanlık",
        icon: MessageCircle,
        subcategories: [
          { name: "Finansal Danışmanlık" },
          { name: "Hukuki Danışmanlık" },
          { name: "İş Danışmanlığı" },
        ],
      },
    ],
  },
  {
    name: "Sanat & Hobi",
    icon: Palette,
    color: "from-pink-500 to-rose-500",
    subcategories: [
      {
        name: "Müzik Enstrümanları",
        icon: Music,
        subcategories: [
          { name: "Gitar" },
          { name: "Piyano" },
          { name: "Davul" },
          { name: "Keman" },
          { name: "Saksafon" },
        ],
      },
      {
        name: "Resim & El Sanatları",
        icon: Brush,
        subcategories: [
          { name: "Yağlı Boya" },
          { name: "Suluboya" },
          { name: "Akrilik Boya" },
          { name: "Pastel Boya" },
          { name: "Kurşun Kalem" },
          { name: "Kömür Kalem" },
          { name: "Fırça" },
          { name: "Palet" },
          { name: "Tuval" },
          { name: "Kağıt" },
          { name: "Çerçeve" },
          { name: "Easel" },
          { name: "Dekoratif Boya" },
          { name: "Seramik" },
          { name: "Cam" },
          { name: "Ahşap" },
          { name: "Metal" },
          { name: "Tekstil" },
          { name: "Dokuma" },
          { name: "Örgü" },
          { name: "Nakış" },
          { name: "Dantel" },
          { name: "Takı Yapımı" },
          { name: "Mum Yapımı" },
          { name: "Sabun Yapımı" },
          { name: "Kokulu Taş" },
          { name: "Mozaik" },
          { name: "Vitray" },
          { name: "Origami" },
          { name: "Quilling" },
          { name: "Scrapbooking" },
        ],
      },
      {
        name: "Koleksiyon",
        icon: Star,
        subcategories: [
          { name: "Pul Koleksiyonu" },
          { name: "Para Koleksiyonu" },
          { name: "Kart Koleksiyonu" },
          { name: "Oyuncak Koleksiyonu" },
        ],
      },
      {
        name: "Model Araçlar",
        icon: Car,
        subcategories: [
          { name: "Model Uçak" },
          { name: "Model Araba" },
          { name: "Model Gemi" },
          { name: "Model Tren" },
        ],
      },
    ],
  },
  {
    name: "Anne & Bebek",
    icon: Baby,
    color: "from-yellow-500 to-orange-500",
    subcategories: [
      {
        name: "Bebek Arabası & Taşıma",
        icon: ShoppingCart,
        subcategories: [
          { name: "Bebek Arabası" },
          { name: "Ana Kucağı" },
          { name: "Kanguru" },
          { name: "Bebek Koltuğu" },
          { name: "Bebek Arabası Koltuğu" },
          { name: "Bebek Arabası Aksesuarları" },
          { name: "Bebek Arabası Örtüsü" },
          { name: "Bebek Arabası Yağmurluğu" },
          { name: "Bebek Arabası Güneşliği" },
          { name: "Bebek Arabası Çantası" },
          { name: "Bebek Arabası Tekerleği" },
          { name: "Bebek Arabası Freni" },
          { name: "Bebek Arabası Kolu" },
          { name: "Bebek Arabası Kancası" },
          { name: "Bebek Arabası İpi" },
          { name: "Bebek Arabası Zinciri" },
          { name: "Bebek Arabası Lastiği" },
          { name: "Bebek Arabası Jantı" },
          { name: "Bebek Arabası Radyosu" },
          { name: "Bebek Arabası Kliması" },
          { name: "Bebek Arabası Isıtıcısı" },
          { name: "Bebek Arabası Soğutucusu" },
          { name: "Bebek Arabası Nemlendiricisi" },
          { name: "Bebek Arabası Arıtıcısı" },
        ],
      },
      {
        name: "Bebek Giyim",
        icon: Shirt,
        subcategories: [
          { name: "Bebek Tulumu" },
          { name: "Bebek Gömleği" },
          { name: "Bebek Pijaması" },
          { name: "Bebek Şapkası" },
        ],
      },
      {
        name: "Oyuncaklar",
        icon: Gift,
        subcategories: [
          { name: "Eğitici Oyuncaklar" },
          { name: "Pelüş Oyuncaklar" },
          { name: "Ahşap Oyuncaklar" },
          { name: "Müzikli Oyuncaklar" },
        ],
      },
      {
        name: "Bebek Maması & Beslenme",
        icon: Baby,
        subcategories: [
          { name: "Bebek Maması" },
          { name: "Biberon" },
          { name: "Mama Sandalyesi" },
          { name: "Beslenme Aksesuarları" },
        ],
      },
    ],
  },
  {
    name: "Oyun & Eğlence",
    icon: Gamepad2,
    color: "from-indigo-500 to-purple-500",
    subcategories: [
      {
        name: "Video Oyunları",
        icon: Gamepad,
        subcategories: [
          { name: "PlayStation" },
          { name: "Xbox" },
          { name: "Nintendo" },
          { name: "PC Oyunları" },
        ],
      },
      {
        name: "Masa Oyunları",
        icon: Users,
        subcategories: [
          { name: "Strateji Oyunları" },
          { name: "Kart Oyunları" },
          { name: "Bilmece Oyunları" },
          { name: "Parti Oyunları" },
        ],
      },
      {
        name: "Oyuncaklar",
        icon: Gift,
        subcategories: [
          { name: "Aksiyon Figürleri" },
          { name: "Bebekler" },
          { name: "Yapı Oyuncakları" },
          { name: "Eğitici Oyuncaklar" },
        ],
      },
    ],
  },
  {
    name: "Sağlık & Güzellik",
    icon: Heart,
    color: "from-red-400 to-pink-400",
    subcategories: [
      {
        name: "Sağlık & Bakım",
        icon: Stethoscope,
        subcategories: [
          { name: "Tekerlekli Sandalye" },
          { name: "Yürüteç" },
          { name: "Koltuk Değneği" },
          { name: "Baston" },
          { name: "Oksijen Cihazı" },
          { name: "Nebulizatör" },
          { name: "Tansiyon Aleti" },
          { name: "Şeker Ölçer" },
          { name: "Termometre" },
          { name: "Steteskop" },
          { name: "Tıbbi Maske" },
          { name: "Eldiven" },
          { name: "Bandaj" },
          { name: "İlaç" },
          { name: "Vitamin" },
          { name: "Protein Tozu" },
          { name: "Omega 3" },
          { name: "Multivitamin" },
          { name: "D Vitamini" },
          { name: "C Vitamini" },
          { name: "B12 Vitamini" },
          { name: "Demir" },
          { name: "Kalsiyum" },
          { name: "Magnezyum" },
          { name: "Çinko" },
        ],
      },
      {
        name: "Güzellik & Kozmetik",
        icon: Scissors,
        subcategories: [
          { name: "Cilt Bakımı" },
          { name: "Saç Bakımı" },
          { name: "Makyaj" },
          { name: "Parfüm" },
        ],
      },
      {
        name: "Bebek & Çocuk Bakımı",
        icon: Baby,
        subcategories: [
          { name: "Bebek Bakımı" },
          { name: "Bebek Maması" },
          { name: "Bez & Islak Mendil" },
          { name: "Bebek Ekipmanları" },
        ],
      },
    ],
  },
  {
    name: "İş & Endüstri",
    icon: Briefcase,
    color: "from-gray-600 to-gray-800",
    subcategories: [
      {
        name: "İş Makineleri",
        icon: Truck,
        subcategories: [
          { name: "Ekskavatör" },
          { name: "Buldozer" },
          { name: "Jeneratör" },
          { name: "Kompresör" },
          { name: "Beton Pompası" },
          { name: "Vinç" },
          { name: "Forklift" },
          { name: "Traktör" },
          { name: "Biçerdöver" },
          { name: "Pulluk" },
          { name: "Tırmık" },
          { name: "Mibzer" },
          { name: "Sulama Sistemi" },
          { name: "Seracılık Ekipmanları" },
          { name: "Hayvancılık Ekipmanları" },
          { name: "Arıcılık Ekipmanları" },
          { name: "Balıkçılık Ekipmanları" },
          { name: "Orman Ekipmanları" },
          { name: "Madencilik Ekipmanları" },
          { name: "Petrol Ekipmanları" },
          { name: "Gaz Ekipmanları" },
          { name: "Su Ekipmanları" },
          { name: "Elektrik Ekipmanları" },
          { name: "Telekomünikasyon Ekipmanları" },
        ],
      },
      {
        name: "Ofis Malzemeleri",
        icon: FileText,
        subcategories: [
          { name: "Ofis Mobilyası" },
          { name: "Ofis Ekipmanları" },
          { name: "Yazıcı & Tarayıcı" },
          { name: "Ofis Aksesuarları" },
        ],
      },
      {
        name: "Endüstriyel Ekipmanlar",
        icon: Wrench,
        subcategories: [
          { name: "Üretim Ekipmanları" },
          { name: "Kalite Kontrol" },
          { name: "Güvenlik Ekipmanları" },
          { name: "Endüstriyel Aletler" },
        ],
      },
    ],
  },
  {
    name: "Seyahat",
    icon: Plane,
    color: "from-sky-500 to-blue-500",
    subcategories: [
      {
        name: "Ulaşım",
        icon: Bus,
        subcategories: [
          { name: "Uçak Bileti" },
          { name: "Otobüs Bileti" },
          { name: "Tren Bileti" },
          { name: "Gemi Bileti" },
        ],
      },
      {
        name: "Konaklama",
        icon: Building,
        subcategories: [
          { name: "Otel Rezervasyonu" },
          { name: "Pansiyon" },
          { name: "Ev Kiralama" },
          { name: "Kamp Alanı" },
        ],
      },
      {
        name: "Tur Paketleri",
        icon: Calendar,
        subcategories: [
          { name: "Yurt İçi Turlar" },
          { name: "Yurt Dışı Turlar" },
          { name: "Günlük Turlar" },
          { name: "Özel Turlar" },
        ],
      },
      {
        name: "Seyahat Aksesuarları",
        icon: ShoppingBag,
        subcategories: [
          { name: "Bavul & Çanta" },
          { name: "Seyahat Organizatörü" },
          { name: "Seyahat Yastığı" },
          { name: "Seyahat Adaptörü" },
        ],
      },
    ],
  },
  {
    name: "Kripto & Finans",
    icon: Bitcoin,
    color: "from-amber-400 to-yellow-500",
    subcategories: [
      {
        name: "Kripto Para",
        icon: Coins,
        subcategories: [
          { name: "Bitcoin" },
          { name: "Ethereum" },
          { name: "Altcoin" },
          { name: "NFT" },
        ],
      },
      {
        name: "Finansal Hizmetler",
        icon: DollarSign,
        subcategories: [
          { name: "Finansal Danışmanlık" },
          { name: "Yatırım Danışmanlığı" },
          { name: "Sigorta" },
          { name: "Kredi" },
        ],
      },
      {
        name: "Yatırım Fırsatları",
        icon: TrendingUp,
        subcategories: [
          { name: "Hisse Senedi" },
          { name: "Tahvil" },
          { name: "Döviz" },
          { name: "Altın & Gümüş" },
        ],
      },
    ],
  },
  {
    name: "Koleksiyon & Değerli Eşyalar",
    icon: Star,
    color: "from-yellow-700 to-amber-400",
    subcategories: [
      {
        name: "Koleksiyonlar",
        icon: Star,
        subcategories: [
          { name: "Pul Koleksiyonu" },
          { name: "Para Koleksiyonu" },
          { name: "Kart Koleksiyonu" },
          { name: "Oyuncak Koleksiyonu" },
          { name: "Antika Eşyalar" },
          { name: "Eski Belgeler" },
          { name: "Madalya & Rozet" },
          { name: "Poster & Afiş" },
          { name: "Plak & Kaset" },
          { name: "Kitap Koleksiyonu" },
          { name: "Dergi Koleksiyonu" },
          { name: "Fosil & Mineral" },
          { name: "Sanat Eseri" },
        ],
      },
      {
        name: "Değerli Eşyalar",
        icon: Gift,
        subcategories: [
          { name: "Altın" },
          { name: "Gümüş" },
          { name: "Pırlanta" },
          { name: "Değerli Taşlar" },
          { name: "Saat (Lüks)" },
          { name: "Mücevher" },
          { name: "Nadir Objeler" },
          { name: "Sanat Eseri" },
          { name: "İmzalı Eşya" },
          { name: "Tarihi Eşya" },
        ],
      },
      {
        name: "Az Bulunanlar",
        icon: Award,
        subcategories: [
          { name: "Nadir Kitap" },
          { name: "Nadir Plak" },
          { name: "Nadir Oyuncak" },
          { name: "Nadir Sanat Eseri" },
          { name: "Nadir Takı" },
          { name: "Nadir Elektronik" },
        ],
      },
    ],
  },
];

const getCategoryPath = (categoryName: string, subCategoryName?: string, subSubCategoryName?: string) => {
  let path = categoryName;
  if (subCategoryName) path += ` > ${subCategoryName}`;
  if (subSubCategoryName) path += ` > ${subSubCategoryName}`;
  return path;
};

const findCategoryByName = (name: string) => {
    if (!name) return null;
    const pathParts = name.split(' > ');
    let currentLevel: any = categoriesConfigEnhanced;
    let foundCategory = null;

    for (const part of pathParts) {
        const category = currentLevel.find((cat: any) => cat.name === part);
        if (category) {
            foundCategory = category;
            currentLevel = category.subcategories || [];
        } else {
            return null; 
        }
    }
    return foundCategory;
};

export { categoriesConfigEnhanced, getCategoryPath, findCategoryByName }; 